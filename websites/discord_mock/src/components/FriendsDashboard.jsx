  ...
  return (
    <div className="flex-1 flex flex-col bg-discord-bg min-w-0">
      ...
      <div className="h-12 px-4 flex items-center shadow-sm border-b border-discord-darker shrink-0">
        <div className="flex items-center mr-4 text-discord-modifier">
        ...
        <div className="h-6 w-[1px] bg-discord-lightest/20 mx-2"></div>

        <div className="flex space-x-4">
          <button
            onClick={() => setActiveTab('online')}
            className={cn("px-2 py-0.5 rounded hover:bg-discord-light hover:text-discord-lightest", activeTab === 'online' ? "text-white bg-discord-light/60" : "text-discord-modifier")}
          >
            Online
          </button>
          <button
            onClick={() => setActiveTab('all')}
            className={cn("px-2 py-0.5 rounded hover:bg-discord-light hover:text-discord-lightest", activeTab === 'all' ? "text-white bg-discord-light/60" : "text-discord-modifier")}
          >
            All
          </button>
          <button
            onClick={() => setActiveTab('pending')}
            className={cn("px-2 py-0.5 rounded hover:bg-discord-light hover:text-discord-lightest", activeTab === 'pending' ? "text-white bg-discord-light/60" : "text-discord-modifier")}
          >
            Pending
            {pendingFriends.length > 0 && (
              <span className="ml-1 bg-discord-red text-white text-[10px] font-bold px-1.5 rounded-full">{pendingFriends.length}</span>
            )}
          </button>
          <button
            onClick={() => setActiveTab('blocked')}
            className={cn("px-2 py-0.5 rounded hover:bg-discord-light hover:text-discord-lightest", activeTab === 'blocked' ? "text-white bg-discord-light/60" : "text-discord-modifier")}
          >
            Blocked
          </button>
          <button
            onClick={() => setActiveTab('add_friend')}
            className={cn("px-2 py-0.5 rounded font-medium text-sm", activeTab === 'add_friend' ? "bg-transparent text-discord-green" : "bg-discord-green text-white")}
          >
            Add Friend
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
          {activeTab === 'add_friend' ? (
            <div>
              ...
              <p className="text-discord-muted text-sm mb-4">You can add friends with their Discord username.</p>
              <div className="flex items-center bg-discord-darker rounded-lg px-4 py-3 border border-discord-divider focus-within:border-discord-blurple">
                <input
                  type="text"
                ...
                <button
                  ...
                  className={cn(
                    ...
                    addFriendInput.trim() ? "bg-discord-blurple hover:bg-discord-blurple/80" : "bg-discord-blurple/50 cursor-not-allowed"
                  )}
                >
                  Send Friend Request
                </button>
              ...
              {addFriendStatus && (
                <p className={cn("text-sm mt-2", addFriendStatus.type === 'success' ? "text-discord-green" : "text-discord-red")}>
            ...
            <div>
              <div className="text-xs font-bold text-discord-modifier uppercase mb-3">
                Pending &#8212; {pendingFriends.length}
              </div>
              {pendingFriends.length === 0 ? (
                <div className="text-center mt-10 text-discord-muted">
                  <p className="text-sm">There are no pending friend requests. Here's Wumpus for now.</p>
                </div>
              ) : (
                <div className="space-y-[1px]">
                  {pendingFriends.map(req => (
                    <div key={req.id} className="flex items-center justify-between p-2.5 rounded hover:bg-discord-light/40 border-t border-discord-modifier/10">
                      <div className="flex items-center">
                        <div className="w-8 h-8 rounded-full bg-discord-light flex items-center justify-center mr-3 text-white font-bold">
                          {req.username.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <div className="font-bold text-white">{req.username}</div>
                          <div className="text-xs text-discord-modifier">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-full bg-discord-light hover:text-white text-discord-modifier"
                        title="Cancel"
                      >
                        <X size={18} />
                      </button>
              ...
              {filteredFriends.length === 0 ? (
                ...
                <div className="space-y-[1px]">
                  {filteredFriends.map(friend => (
                    <div key={friend.id} className="flex items-center justify-between p-2.5 rounded hover:bg-discord-light/40 border-t border-discord-modifier/10">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-discord-modifier">Blocked</div>
                        </div>
                      </div>
                      <button
                        onClick={() => store.unblockUser(friend.id)}
                        className="px-3 py-1 text-sm bg-discord-light hover:bg-discord-selected text-white rounded"
                      >
                        Unblock
                      </button>
                    </div>
            ...
            <>
              <div className="mb-4 relative">
                <input
                  ...
                  className="w-full bg-discord-darker text-discord-lightest px-3 py-2 rounded outline-none"
                />
                <Search className="absolute right-3 top-2.5 text-discord-modifier" size={20} />
              </div>
              ...
              <div className="space-y-[1px]">
                {filteredFriends.map(friend => (
                  <div
                    ...
                    className="group flex items-center justify-between p-2.5 rounded hover:bg-discord-light/40 cursor-pointer border-t border-discord-modifier/10"
                  >
                    <div className="flex items-center">
                      <div className="relative mr-3">
                        <img src={friend.avatar} alt={friend.username} className="w-8 h-8 rounded-full" />
                        <div className={cn(
                          ...
                          friend.status === 'dnd' ? "bg-discord-dnd" : "bg-discord-offline"
                        )} />
                      </div>
                      <div>
                        <div className="font-bold text-white flex items-center">
                          {friend.username}
                          <span className="hidden group-hover:inline text-xs text-discord-modifier ml-1">#{friend.discriminator}</span>
                        </div>
                        <div className="text-xs text-discord-modifier">
                    ...
                    <div className="flex items-center space-x-2 bg-discord-darker p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        className="p-1.5 rounded-full bg-discord-light hover:text-white text-discord-modifier"
                        title="Message"
        ...
        {/* Active Now Sidebar */}
        <div className="w-[360px] border-l border-discord-darker p-4 hidden xl:block">
          ...
          {activeUsers.length > 0 ? (
            <div className="space-y-3">
              {activeUsers.map(user => (
                <div
                  key={user.id}
                  className="flex items-center p-2 rounded hover:bg-discord-light/40 cursor-pointer"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-discord-muted truncate">{user.customStatus}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="text-center mt-10">
              ...
              <div className="text-sm text-discord-modifier px-4">
  ...
  return (
    <div style={menuStyle} className="min-w-[188px] bg-[#111214] rounded-lg shadow-xl border border-[#2e2f34] py-1.5" onClick={e => e.stopPropagation()}>
      ...
      <button
        ...
        className="w-full text-left px-3 py-1.5 text-sm text-discord-red hover:bg-discord-red hover:text-white rounded mx-1.5"
        style={{ width: 'calc(100% - 12px)' }}
      >
        Remove Friend
      </button>
