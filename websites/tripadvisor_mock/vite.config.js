...
function createApiHandlers(server) {
  ...
  server.middlewares.use('/post', async (req, res, next) => {
    ...
    try {
      ...
      if (action === 'set_current') {
        const previousState = readState(sid)
        writeInitialStateIfMissing(sid, previousState || data.initial_state || data.state)
  ...
  server.middlewares.use('/go', (req, res, next) => {
    ...
    writeInitialStateIfMissing(sid, initial)
...
export default defineConfig({
  ...
  plugins: [
    ...
    {
      name: 'mock-api',
      configureServer(server) { createApiHandlers(server) },
      configurePreviewServer(server) { createApiHandlers(server) }
    }
  ],
