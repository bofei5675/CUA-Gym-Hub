...
function getStateFile(sid) {
  ...
  return path.join(STATE_DIR, `${sid.replace(/[^a-zA-Z0-9_-]/g, '')}.json`)
}

function getInitialStateFile(sid) {
  if (!sid) return path.join(process.cwd(), '.mock-state.initial.json')
  return path.join(STATE_DIR, `${sid.replace(/[^a-zA-Z0-9_-]/g, '')}.initial.json`)
}
...
function clearState(sid) {
  try {
    ...
    const initF = getInitialStateFile(sid)
    if (fs.existsSync(initF)) fs.unlinkSync(initF)
    return true
  } catch { return false }
}
...
function createMiddleware(server) {
  ...
  server.middlewares.use('/files', (req, res, next) => {
    ...
    const mimeMap = { '.pdf': 'application/pdf', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.txt': 'text/plain', '.csv': 'text/csv', '.zip': 'application/zip', '.py': 'text/x-python', '.java': 'text/x-java', '.pptx': 'application/vnd.openxmlformats-officedocument.presentationml.presentation', '.docx': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }
  ...
  server.middlewares.use('/post', async (req, res, next) => {
    ...
    try {
      ...
      if (action === 'set') {
        ...
        const newState = data.merge ? deepMerge(cur, data.state) : data.state
        writeState(sid, newState)
        if (!readInitialState(sid)) {
          try { fs.writeFileSync(getInitialStateFile(sid), JSON.stringify(newState, null, 2)) } catch {}
        }
        res.setHeader('Content-Type', 'application/json')
      ...
      if (action === 'set_current') {
        ...
        writeState(sid, newState)
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify({ success: true, message: 'Current state updated.' }))
        return
      }
      res.statusCode = 400
  ...
  server.middlewares.use('/state', (req, res, next) => {
    ...
    res.end(JSON.stringify({ stored_state: st, has_custom_state: st !== null, sid }))
  })
  ...
  server.middlewares.use('/go', (req, res, next) => {
    ...
    const initialSt = readInitialState(sid)
    const initial = initialSt || currentState || {}
...
export default defineConfig({
  plugins: [
    react(),
    {
      name: 'canvas-lms-mock-api',
      configureServer(server) { createMiddleware(server) },
      configurePreviewServer(server) { createMiddleware(server) }
    }
  ],
  server: {
