...
import { createInitialState } from './src/store/initialData.js'
...
function writeInitialState(sid, state) {
  try { fs.writeFileSync(getInitialStateFile(sid), JSON.stringify(state, null, 2)); return true } catch (e) { console.error('Error writing initial state:', e); return false }
}

function clearState(sid) {
  try {
    const file = getStateFile(sid); if (fs.existsSync(file)) fs.unlinkSync(file)
    const initialFile = getInitialStateFile(sid); if (fs.existsSync(initialFile)) fs.unlinkSync(initialFile)
    return true
    ...
    react(),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use('/go', (req, res, next) => {
          ...
          const defaultData = createInitialState()
