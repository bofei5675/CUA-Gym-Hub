        ...
        return (
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-4 mb-6 items-center text-sm">
              <span className="text-github-muted">base:</span>
              <select
                value={baseBranch}
                onChange={e => setBaseBranch(e.target.value)}
                className="bg-[#21262d] border border-github-border rounded-md px-2 py-1 text-sm text-github-text"
              >
                {branches.map(b => <option key={b.id} value={b.name}>{b.name}</option>)}
                {branches.length === 0 && <option value="main">main</option>}
              </select>
              <span className="text-github-muted">←</span>
              <span className="text-github-muted">compare:</span>
              <select
                value={compareBranch}
            ...
            <form onSubmit={handleCreatePR} className="bg-github-card border border-github-border rounded-md p-4">
              <div className="mb-4">
                <input
                  type="text"
                  placeholder="Title"
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 text-sm text-github-text cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={isDraft}
                    onChange={e => setIsDraft(e.target.checked)}
                    className="accent-github-muted rounded"
                  />
                  Create as draft
                </label>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setShowNewPRForm(false)} className="px-4 py-2 text-github-muted hover:text-github-text">Cancel</button>
                  <button type="submit" className={`px-4 py-2 font-semibold rounded-md hover:bg-opacity-90 ${isDraft ? 'bg-[#30363d] text-white border border-github-border' : 'bg-github-success text-white'}`}>
                  ...
                  <div key={pr.id} className="p-3 hover:bg-[#161b22] flex items-start gap-2 group">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <Link to={`${pr.number}`} className="font-semibold text-white hover:text-github-accent hover:underline cursor-pointer">
                          {pr.title}
                        </Link>
                        {(pr.labels || []).map(label => (
