...
function readState(sid) {
  try { const f = getStateFile(sid); if (fs.existsSync(f)) return JSON.parse(fs.readFileSync(f, 'utf-8')) } catch (e) { console.error('readState error:', e) }
  return null
}
...
function setupMiddleware(server) {
  server.middlewares.use('/post', async (req, res, next) => {
    ...
    try {
      ...
      if (action === 'set') {
        const newState = data.merge ? deepMerge(readState(sid) || {}, data.state) : data.state
