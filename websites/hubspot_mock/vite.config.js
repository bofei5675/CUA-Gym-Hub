...
function ensureInitialState(sid,state) { if(!readInitialState(sid)) writeInitialState(sid,state||{}) }
      ...
      configureServer(server) {
        ...
        server.middlewares.use('/post', async (req, res, next) => {
          ...
          try {
            ...
            if (action === 'reset') { clearState(sid); clearInitialState(sid); res.setHeader('Content-Type','application/json'); res.end(JSON.stringify({success:true,sid,message:'State reset to default.'})); return }
            if (action === 'set') { const cur=readState(sid)||{}; const ns=data.merge?deepMerge(cur,data.state):data.state; ensureInitialState(sid,ns); writeState(sid,ns); res.setHeader('Content-Type','application/json'); res.end(JSON.stringify({success:true,sid,message:'State updated.',state:ns})); return }
