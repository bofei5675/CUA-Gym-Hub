  ...
  return {
    ...
    configureServer(server) {
        ...
        try {
          ...
          if (action === 'set') {
            ...
            if (!readInitialState(sid)) {
              try { fs.writeFileSync(getInitialStateFile(sid), JSON.stringify(newState, null, 2)) } catch (e) { console.error('Error writing initial state:', e) }
            }
            res.setHeader('Content-Type', 'application/json')
            res.setHeader('Cache-Control', 'no-cache, no-store')
            res.end(JSON.stringify({ success: true, message: 'State updated. Refresh browser.', state: newState }))
            return
          }
      ...
      server.middlewares.use('/go', (req, res, next) => {
        if (req.method !== 'GET') return next()
        if (req.url && !req.url.startsWith('/?') && req.url !== '/' && !req.url.startsWith('?')) return next()
        const query = parseQuery(req.url || '')
        const sid = query.sid || null
        const currentState = readState(sid)
    ...
    configurePreviewServer(server) {
      server.middlewares.use('/upload', async (req, res, next) => {
        ...
        const contentType = req.headers['content-type'] || ''
        const boundaryMatch = contentType.match(/boundary=(.+)/)
        if (!boundaryMatch) { res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'multipart/form-data required' })); return }
        const chunks = []; for await (const chunk of req) chunks.push(chunk)
        const buf = Buffer.concat(chunks)
        const files = parseMultipart(buf, boundaryMatch[1])
        if (files.length === 0) { res.statusCode = 400; res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ error: 'No files' })); return }
        const filesDir = getFilesDir(sid)
        const uploaded = []
        for (const file of files) {
          const safeFilename = file.filename.replace(/[^a-zA-Z0-9._-]/g, '_')
      ...
      server.middlewares.use('/post', async (req, res, next) => {
        ...
        try {
          const data = JSON.parse(body)
          const action = data.action || 'set'
          if (action === 'reset') { clearState(sid); res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ success: true })); return }
          if (action === 'set') {
            const cs = readState(sid) || {}
            const ns = data.merge ? deepMerge(cs, data.state) : data.state
            writeState(sid, ns)
            if (!readInitialState(sid)) { try { fs.writeFileSync(getInitialStateFile(sid), JSON.stringify(ns, null, 2)) } catch (e) {} }
            res.setHeader('Content-Type', 'application/json'); res.end(JSON.stringify({ success: true, state: ns })); return
          }
          if (action === 'set_current') {
            const cs = readState(sid) || {}
            const ns = data.merge ? deepMerge(cs, data.state) : data.state
