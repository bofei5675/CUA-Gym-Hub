...
function createApiMiddleware(server) {
  ...
  server.middlewares.use('/files', (req, res, next) => {
    ...
    const mimeMap = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.txt': 'text/plain', '.csv': 'text/csv', '.pdf': 'application/pdf' }
  ...
  server.middlewares.use('/post', async (req, res, next) => {
    ...
    try {
      ...
      if (action === 'set') {
        ...
        try { fs.writeFileSync(getInitialStateFile(sid), JSON.stringify(data.state, null, 2)) }
        catch (e) { console.error('Error writing initial state:', e) }
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify({ success: true, sid, message: 'State updated.', state: data.state }))
        return
      }
      if (action === 'set_current') {
        const current = readState(sid) || {}
        writeInitialStateIfMissing(sid, Object.keys(current).length > 0 ? current : data.state)
        writeState(sid, data.state)
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify({ success: true, message: 'Current state updated.', state: data.state }))
        return
      }
      res.statusCode = 400
