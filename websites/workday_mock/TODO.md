...
- [x] **Visual design system overhaul**: The current mock uses orange (#FF6B35) as primary color but real Workday uses **blue** (#0875E1) as primary. Update `tailwind.config.js`: change `primary` to `#0875E1`, `primary-hover` to `#0660C1`, add `workday-orange` as `#F68D2E` for accent/logo only, `sidebar-blue` as `#003A70` for profile panel, `light-blue` as `#E8F2FC` for active states. Update all components that reference `bg-primary`, `text-primary`, etc. — these should now render as blue, not orange. Study `assets/screenshots/000002.jpg` and `profile_000001.jpg` for the exact Workday color feel.
- [x] **Header redesign to match Workday**: Redesign `Header.jsx` to match real Workday top bar (see `assets/screenshots/000002.jpg`): white background, left side has hamburger menu icon (☰) + blue "W" logo with orange arc, center has a wide search bar with "Search" placeholder and magnifying glass icon, right side has help icon (?), notification bell with red badge count, and circular profile photo. Current search bar is close but needs the hamburger menu on the left and the Workday logo style.
- [x] **Sidebar redesign to Workday navigation pattern**: Real Workday uses a **top bar only** on most pages (no persistent left sidebar). The sidebar is a slide-out panel triggered by the hamburger menu. However, for our mock, keep the persistent left sidebar for easy navigation but restyle it: white background, group items with section headers ("MENU", "MY INFORMATION"), use blue highlight for active item (left border + light blue background like `#E8F2FC`), add the "W" Workday logo at top with orange arc accent. Remove the "Sign Out" button (out of scope).
...
- [x] **Workday apps grid on dashboard**: Add an "Apps" section at bottom of dashboard showing a 4×2 grid of icon tiles (like Workday worklets). Each tile: 64px icon circle, label below. Icons: Pay, Benefits, Time Off, Directory, Performance, Profile, Inbox, Help. Each links to corresponding route.
...
- Recruiting module
- Learning Management System (separate Workday product)
