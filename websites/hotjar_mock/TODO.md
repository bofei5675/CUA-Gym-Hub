...
- [x] **Header bar**: Left: Hotjar flame logo (orange/red `#FF3C00` stylized flame SVG inline, linking to `/`). Next to logo: site selector dropdown showing current site name + URL with chevron-down, clicking opens dropdown listing all sites from state. Center-right cluster: settings gear icon, invite-user icon, notification bell icon (with blue dot badge), help question-mark icon, user avatar circle (40px, uses `ui-avatars.com` URL from currentUser). See `reference/botubot_0003.webp` header, `reference/botubot_0005.webp` header for exact layout.
