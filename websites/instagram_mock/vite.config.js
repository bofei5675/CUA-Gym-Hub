...
import { generateInitialData } from './src/utils/mockData.js'
...
function clearState(sid) { try{const f=getStateFile(sid);if(fs.existsSync(f))fs.unlinkSync(f);const init=getInitialStateFile(sid);if(fs.existsSync(init))fs.unlinkSync(init);return true}catch(e){return false} }
function getInitialStateFile(sid) { if(!sid)return path.join(process.cwd(),'.mock-state.initial.json');const safeSid=sid.replace(/[^a-zA-Z0-9_-]/g,'');return path.join(STATE_DIR,`${safeSid}.initial.json`) }
function readInitialState(sid) { try{const f=getInitialStateFile(sid);if(fs.existsSync(f))return JSON.parse(fs.readFileSync(f,'utf-8'))}catch(e){console.error('Error reading initial state:',e)}return null }
function writeInitialStateIfMissing(sid,state) { if(readInitialState(sid)||!state)return;try{fs.writeFileSync(getInitialStateFile(sid),JSON.stringify(state,null,2))}catch(e){console.error('Error writing initial state file:',e)} }
function calculateStateDiff(initial,current) { const diff={};for(const key in current){if(!initial||JSON.stringify(current[key])!==JSON.stringify(initial[key])){if(!diff[key])diff[key]={};if(!initial||!initial[key]){diff[key].added=current[key]}else{diff[key].modified=current[key]}}}return Object.keys(diff).length>0?diff:{} }
...
function serveUploadedFile(req, res, next) {
  ...
  const mimeMap = { '.pdf': 'application/pdf', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.gif': 'image/gif', '.webp': 'image/webp', '.txt': 'text/plain', '.csv': 'text/csv', '.zip': 'application/zip', '.doc': 'application/msword', '.docx': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', '.xls': 'application/vnd.ms-excel', '.xlsx': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', '.mp4': 'video/mp4', '.mov': 'video/quicktime', '.webm': 'video/webm' }
  ...
  plugins: [
    react(),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use('/files', serveUploadedFile)
        ...
        server.middlewares.use('/post', async (req, res, next) => {
          ...
          try {
            ...
            if (action === 'set') {
              ...
              res.setHeader('Content-Type','application/json');
              res.end(JSON.stringify({success:true,sid,message:'State updated.',state:ns}));
              return
            }
        ...
        server.middlewares.use('/go', (req, res, next) => {
          ...
          const initial = initialState || currentState || generateInitialData(); const current = currentState || initial
