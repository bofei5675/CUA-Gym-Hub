...
Workspace (1) ──> (N) Board
Board (1) ──> (N) Group
Board (1) ──> (N) Column (defines schema)
Group (1) ──> (N) Item
Item (1) ──> (N) ColumnValue (data per column)
Item (1) ──> (N) Subitem
Item (1) ──> (N) Update (comments/activity)
User ──referenced by──> People columns, Updates, Notifications
```
...
Represents a team member in the workspace.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"user-1"` | Unique identifier |
| `name` | string | `"Sarah Johnson"` | Full name |
| `email` | string | `"sarah@company.com"` | Email address |
| `avatar` | string | `"/avatars/sarah.jpg"` | Avatar image URL or initials fallback |
| `initials` | string | `"SJ"` | Two-letter initials for avatar fallback |
| `color` | string | `"#0073EA"` | Personal color for avatar backgrounds |
| `role` | string | `"Admin"` | Role: "Admin", "Member", "Viewer" |
| `isCurrentUser` | boolean | `true` | Whether this is the logged-in user |
...
| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"ws-1"` | Unique identifier |
| `name` | string | `"Main Workspace"` | Workspace name |
| `icon` | string | `"M"` | Single letter or emoji icon |
| `color` | string | `"#0073EA"` | Workspace icon background color |
| `boardIds` | string[] | `["board-1", "board-2"]` | Boards in this workspace |

### 3. Board

The primary data container — a customizable table.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"board-1"` | Unique identifier |
| `name` | string | `"Team Projects"` | Board display name |
| `description` | string | `"Track all team projects and tasks"` | Optional board description |
| `workspaceId` | string | `"ws-1"` | Parent workspace |
| `type` | string | `"board"` | "board", "dashboard", "doc" |
| `isFavorite` | boolean | `false` | Starred/favorited by current user |
| `groupIds` | string[] | `["group-1", "group-2"]` | Ordered group IDs |
| `columnIds` | string[] | `["col-status", "col-person"]` | Ordered column IDs (schema definition) |
| `views` | object[] | See below | Available views for this board |
| `createdAt` | string | `"2024-11-01T10:00:00Z"` | ISO creation date |
| `updatedAt` | string | `"2025-01-15T14:30:00Z"` | ISO last updated |

**Board Views** (embedded in board):
```js
views: [
  { id: "view-1", type: "table", name: "Table", isDefault: true },
  { id: "view-2", type: "kanban", name: "Kanban", statusColumnId: "col-status" },
]
```

### 4. Column

Defines a column type and configuration for a board (schema-level, not data).

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"col-status"` | Unique identifier |
| `title` | string | `"Status"` | Column header text |
| `type` | string | `"status"` | Column type (see types below) |
| `width` | number | `150` | Column width in pixels |
| `settings` | object | See below | Type-specific settings |

**Column Types & Settings:**

| Type | `settings` Object | Description |
|------|-------------------|-------------|
| `"status"` | `{ labels: { 0: { text: "Done", color: "#00C875" }, 1: { text: "Working on it", color: "#FDAB3D" }, 2: { text: "Stuck", color: "#E2445C" }, 3: { text: "Not Started", color: "#C4C4C4" } } }` | Status labels with colors |
| `"people"` | `{}` | People/assignee picker |
| `"date"` | `{}` | Single date |
| `"timeline"` | `{}` | Date range (start + end) |
| `"numbers"` | `{ unit: "$", direction: "left" }` | Numeric with optional unit |
| `"text"` | `{}` | Plain text |
| `"priority"` | `{ labels: { 0: { text: "Critical", color: "#333333" }, 1: { text: "High", color: "#401694" }, 2: { text: "Medium", color: "#5559DF" }, 3: { text: "Low", color: "#579BFC" }, 4: { text: "", color: "#C4C4C4" } } }` | Priority labels |
| `"dropdown"` | `{ options: ["Option 1", "Option 2"] }` | Dropdown selector |
| `"checkbox"` | `{}` | Boolean checkbox |
| `"rating"` | `{ max: 5 }` | Star rating |
| `"tags"` | `{ options: [{ id: "tag-1", name: "Frontend", color: "#0073EA" }] }` | Colored tags |
| `"link"` | `{}` | URL link |
| `"email"` | `{}` | Email address |
...
A colored section within a board that organizes items.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"group-1"` | Unique identifier |
| `boardId` | string | `"board-1"` | Parent board |
| `title` | string | `"This Quarter"` | Group display name |
| `color` | string | `"#579BFC"` | Group accent color (left bar + header text) |
| `isCollapsed` | boolean | `false` | Collapsed/expanded state |
| `itemIds` | string[] | `["item-1", "item-2"]` | Ordered item IDs in this group |
| `position` | number | `0` | Sort order within board |

### 6. Item

A single row in a board group.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"item-1"` | Unique identifier |
| `boardId` | string | `"board-1"` | Parent board |
| `groupId` | string | `"group-1"` | Parent group |
| `name` | string | `"Design landing page"` | Item display name |
| `columnValues` | object | `{ "col-status": { value: 1 }, ... }` | Key-value map of column ID → value |
| `subitemIds` | string[] | `[]` | Child subitem IDs |
| `isSelected` | boolean | `false` | Checkbox selection state |
| `createdAt` | string | `"2024-12-01T09:00:00Z"` | ISO creation date |
| `updatedAt` | string | `"2025-01-10T16:00:00Z"` | ISO last updated |
| `creatorId` | string | `"user-1"` | Creator user ID |

**Column Value Formats** (stored in `item.columnValues[columnId]`):

| Column Type | Value Format | Example |
|-------------|-------------|---------|
| `status` | `{ value: <labelIndex> }` | `{ value: 0 }` → "Done" |
| `people` | `{ value: ["user-1", "user-3"] }` | Array of user IDs |
| `date` | `{ value: "2025-02-15" }` | ISO date string |
| `timeline` | `{ value: { start: "2025-01-01", end: "2025-01-31" } }` | Date range |
| `numbers` | `{ value: 42000 }` | Numeric value |
| `text` | `{ value: "Some text here" }` | Plain string |
| `priority` | `{ value: <labelIndex> }` | `{ value: 1 }` → "High" |
| `dropdown` | `{ value: "Option 2" }` | Selected option string |
| `checkbox` | `{ value: true }` | Boolean |
| `rating` | `{ value: 4 }` | Number 0-5 |
| `tags` | `{ value: ["tag-1", "tag-3"] }` | Array of tag IDs |
| `link` | `{ value: { url: "https://...", text: "Link" } }` | URL + display text |
| `email` | `{ value: "alice@co.com" }` | Email string |
| `progress` | `{ value: 67 }` | Percentage 0-100 |

### 7. Update (Comment/Activity)

A comment or activity entry on an item.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"update-1"` | Unique identifier |
| `itemId` | string | `"item-1"` | Parent item |
| `authorId` | string | `"user-2"` | Author user ID |
| `body` | string | `"Updated the mockups..."` | Text content (supports basic formatting) |
| `createdAt` | string | `"2025-01-10T14:30:00Z"` | ISO creation timestamp |
| `likes` | string[] | `["user-1"]` | User IDs who liked this update |
| `isReply` | boolean | `false` | Whether this is a reply to another update |
| `parentUpdateId` | string \| null | `null` | ID of parent update if reply |

### 8. Notification

A notification entry for the notification bell.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"notif-1"` | Unique identifier |
| `type` | string | `"mention"` | "mention", "assignment", "status_change", "update" |
| `message` | string | `"Alex mentioned you..."` | Notification text |
| `userId` | string | `"user-1"` | Recipient user ID |
| `actorId` | string | `"user-2"` | User who triggered the notification |
| `itemId` | string | `"item-3"` | Related item |
| `boardId` | string | `"board-1"` | Related board |
| `isRead` | boolean | `false` | Read/unread state |
| `createdAt` | string | `"2025-01-15T10:00:00Z"` | ISO timestamp |

### 9. Activity Log Entry

Tracks changes for the board activity log.

| Field | Type | Example | Description |
|-------|------|---------|-------------|
| `id` | string | `"activity-1"` | Unique identifier |
| `boardId` | string | `"board-1"` | Board where change occurred |
| `itemId` | string | `"item-2"` | Item that was changed |
| `userId` | string | `"user-1"` | User who made the change |
| `columnId` | string \| null | `"col-status"` | Column that was changed (null for item-level) |
| `action` | string | `"column_change"` | "create", "column_change", "move", "archive", "delete" |
| `previousValue` | any | `{ value: 3 }` | Value before change |
| `newValue` | any | `{ value: 0 }` | Value after change |
| `createdAt` | string | `"2025-01-15T09:30:00Z"` | ISO timestamp |
...
function createInitialData() {
  return {
    ...
    // Columns (definitions/schema)
    columns: {
      // Board 1: Team Projects
    ...
    // Items (30 items across 4 boards)
    items: {
      // Board 1: Team Projects — This Quarter
      ...
      // Board 1: Team Projects — Next Quarter
      ...
      // Board 1: Team Projects — Completed
...
1. **All entities are stored in flat maps** (keyed by ID) for O(1) lookups. Relationships use ID arrays.
2. **Column definitions are shared** — stored once in `columns`, referenced by `board.columnIds`.
3. **Column values are polymorphic** — the `value` field type depends on the column `type`.
4. **Status/Priority use label indices** — `{ value: 0 }` maps to the first label in `column.settings.labels`.
5. **Groups order items** — the `group.itemIds` array defines display order within that group.
6. **Boards order groups** — the `board.groupIds` array defines display order; `group.position` is a secondary sort.
7. **Current user is always `user-1` (Sarah Johnson)** — no login required.
