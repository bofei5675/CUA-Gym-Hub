    ...
    >
      {({ isActive }) => (
        <>
          ...
          {isActive && (
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-zoom-blue rounded-full" />
          )}
        </>
      )}
    </NavLink>
...
export default function Layout() {
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* Top Navigation Bar */}
      <nav className="h-12 bg-white border-b border-zoom-border flex items-center px-3 shrink-0 z-20">
        {/* Left section: logo + nav arrows + search */}
        <div className="flex items-center space-x-2 min-w-[260px]">
          {/* Zoom logo */}
          <div className="w-8 h-8 bg-zoom-blue rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0">
            Z
          </div>
          {/* Nav arrows */}
          <button
            onClick={() => window.history.back()}
            className="p-1 text-zoom-gray hover:text-zoom-dark hover:bg-gray-100 rounded transition-colors"
            title="Go back"
          >
          ...
          <div ref={searchRef} className="relative ml-1">
            {showSearchBar ? (
              <div className="flex items-center bg-[#F1F1F1] rounded-full px-3 py-1.5 w-[220px]">
                <Search className="w-3.5 h-3.5 text-zoom-gray mr-2 shrink-0" />
                <input
                  ref={searchInputRef}
                  type="text"
                  value={searchQuery}
                  ...
                  placeholder="Search..."
                  className="flex-1 bg-transparent text-xs outline-none text-zoom-dark placeholder:text-zoom-gray"
                  onKeyDown={e => { if (e.key === 'Escape') { setShowSearchBar(false); setSearchQuery(''); } }}
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="ml-1 text-zoom-gray hover:text-zoom-dark">
                    <X className="w-3 h-3" />
                  </button>
                )}
              </div>
              ...
              >
                <Search className="w-3.5 h-3.5 text-zoom-gray mr-2 shrink-0" />
                <span className="text-xs text-zoom-gray">Search</span>
                <span className="ml-auto text-[10px] text-zoom-gray bg-white rounded px-1 py-0.5 border border-zoom-border">Ctrl+F</span>
              </button>
            )}

            {/* Search results dropdown */}
            {showSearchBar && searchQuery.trim() && (
              <div className="absolute top-full left-0 mt-1 bg-white border border-zoom-border rounded-lg shadow-lg w-72 z-50 overflow-hidden">
                {searchResults.length === 0 ? (
                  <div className="px-4 py-3 text-sm text-zoom-gray">No results found</div>
                ) : (
                  <div className="py-1">
                    {searchResults.map((result, i) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-zoom-gray rounded uppercase font-semibold tracking-wide shrink-0">
                            {result.type}
                          </span>
                          <div className="min-w-0">
                            <div className="text-sm font-medium text-zoom-dark truncate">{result.label}</div>
                            {result.sub && <div className="text-xs text-zoom-gray truncate">{result.sub}</div>}
                          </div>
                        </div>
                      </button>
                    ))}
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                ...
                isActive ? 'text-zoom-blue bg-zoom-hover' : 'text-zoom-gray hover:text-zoom-dark hover:bg-gray-100'
              )
            }
          >
          ...
          <div ref={statusRef} className="relative">
            ...
            >
              <img
                src={user.avatar}
                alt="Profile"
                className="w-8 h-8 rounded-full border border-zoom-border cursor-pointer hover:opacity-90 transition-opacity"
              />
              {/* Status dot */}
              <div className={cn(
            ...
            {showStatusDropdown && (
              <div className="absolute top-full right-0 mt-1 bg-white border border-zoom-border rounded-xl shadow-lg w-52 py-1 z-50">
                <div className="px-3 py-2 border-b border-gray-100">
                  <div className="font-medium text-sm text-zoom-dark truncate">{user.username}</div>
                  <div className="text-xs text-zoom-gray truncate">{user.email}</div>
                </div>
                <div className="py-1">
                  <div className="px-3 py-1 text-[10px] text-zoom-gray font-semibold uppercase tracking-wider">Status</div>
                  {STATUS_OPTIONS.map(s => (
                    <button
                      ...
                      className={cn(
                        ...
                        user.status === s.value && 'bg-zoom-hover'
                      )}
                    >
                      <div className={cn('w-2.5 h-2.5 rounded-full shrink-0', s.color)} />
                      <span className={user.status === s.value ? 'font-medium text-zoom-blue' : 'text-zoom-dark'}>{s.label}</span>
                      {user.status === s.value && <span className="ml-auto text-zoom-blue text-xs">✓</span>}
                    </button>
                  ))}
                </div>
                <div className="border-t border-gray-100 pt-1">
                  <NavLink
                    ...
                    className="w-full text-left px-3 py-2 text-sm text-zoom-dark hover:bg-gray-50 transition-colors flex items-center"
                  >
                    <Settings className="w-3.5 h-3.5 mr-2 text-zoom-gray" />
                    Settings
                  </NavLink>
                </div>
              </div>
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-auto bg-zoom-bg">
        <Outlet />
      </main>
    </div>
  );
