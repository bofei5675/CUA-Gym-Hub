...
function readState(sid: string | null) {
  try {
...
function readInitialState(sid: string | null) {
  try {
  ...
  plugins: [
    react(),
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use('/post', async (req: any, res: any, next: any) => {
          if (req.method !== 'POST') {
            return next();
          }
        ...
        server.middlewares.use('/state', (req: any, res: any, next: any) => {
          if (req.method !== 'GET') {
            return next();
          }
        ...
        server.middlewares.use('/go', (req: any, res: any, next: any) => {
          if (req.method !== 'GET') {
            return next();
          }

          // If the browser is requesting an HTML page, let the SPA handle it
          // so the React StateInspector component renders with live Zustand state
