...
export const Starred = () => {
  const { state, getStarredItems, dispatch } = useFileSystem();
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      <div className="h-14 border-b border-[#DADCE0] flex items-center justify-between px-4 flex-shrink-0">
        <h1 className="text-base font-normal text-[#202124]">Starred</h1>
      ...
      <div className="flex-1 overflow-y-auto">
        {items.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-[#5F6368] gap-3">
            <Star className="w-16 h-16 text-[#DADCE0]" />
            <p className="text-base font-medium">No starred files</p>
            <p className="text-sm text-[#9AA0A6]">Add stars to things that you want to find easily later</p>
          </div>
        ) : (
          state.viewMode === 'grid'
