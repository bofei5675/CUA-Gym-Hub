...
- [x] **Funnel promo card**: "Measure your first funnel in Amplitude" heading with info icon, "Last 90 Days" subtitle, "Funnel Analysis" link. Shows a mini funnel bar chart (2 bars, 100% and ~25%). Clicking "Funnel Analysis" navigates to `/chart/new?type=funnel`. See botubot_0012.
...
- [x] **Retention tab**: When "Retention" tab is active, left panel shows start event and return event selectors. Right panel: retention curve line chart (percentage y-axis 0%-100%, Day 0 through Day 30 x-axis), steep initial drop then flattening. Below chart: retention grid table (cohort rows x day columns) with percentage values, color-coded cells (darker = higher retention). Data from RetentionChart entity. See botubot_0042 (Ask Amplitude shows retention chart format).
...
- AI inference for Ask Amplitude (static mock responses)
