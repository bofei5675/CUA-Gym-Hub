    ...
    export default function Milestones() {
      ...
      return (
        <div className="max-w-6xl mx-auto">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-4">
              <Link
                ...
                className="text-sm text-github-muted hover:text-github-accent"
              >
                &larr; Back to Issues
              </Link>
            </div>
          ...
          {/* New Milestone Form */}
          {showNewForm && (
            <form onSubmit={handleCreateMilestone} className="bg-github-card border border-github-border rounded-md p-4 mb-6">
              <h3 className="text-lg font-semibold text-white mb-4">New milestone</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-github-text mb-1">Title</label>
                  <input
                    type="text"
                    value={newTitle}
                    onChange={e => setNewTitle(e.target.value)}
                    placeholder="Title"
                    className="w-full bg-[#0d1117] border border-github-border rounded-md px-3 py-2 text-github-text text-sm focus:ring-2 focus:ring-github-accent outline-none"
                    required
                  />
                </div>
                <div>
                  <label className="block text-sm text-github-text mb-1">Due date (optional)</label>
                  <input
                    type="date"
                    value={newDueDate}
                    onChange={e => setNewDueDate(e.target.value)}
                    className="bg-[#0d1117] border border-github-border rounded-md px-3 py-2 text-github-text text-sm focus:ring-2 focus:ring-github-accent outline-none"
                  />
                </div>
                <div>
                  <label className="block text-sm text-github-text mb-1">Description (optional)</label>
                  <textarea
                    value={newDescription}
                    onChange={e => setNewDescription(e.target.value)}
                    placeholder="Description"
                    className="w-full h-24 bg-[#0d1117] border border-github-border rounded-md px-3 py-2 text-github-text text-sm focus:ring-2 focus:ring-github-accent outline-none resize-y"
                  />
                </div>
                <div className="flex gap-2 justify-end">
                  <button type="button" onClick={() => setShowNewForm(false)} className="px-4 py-1.5 text-github-muted text-sm hover:text-white">
                    Cancel
                  </button>
          ...
          <div className="bg-github-card border border-github-border rounded-md overflow-hidden">
            <div className="bg-[#161b22] border-b border-github-border p-3 flex items-center gap-4 text-sm">
              <button
                  ...
                  <div key={ms.id} className="p-4 hover:bg-[#161b22]">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-xs text-github-muted mt-1">
                      ...
                      <button
                        ...
                        className="text-xs px-3 py-1 border border-github-border rounded-md text-github-muted hover:text-white hover:bg-[#30363d]"
                      >
                        {ms.state === 'open' ? 'Close' : 'Reopen'}
                      </button>
                    </div>
                    {ms.description && (
                      <p className="text-sm text-github-muted mb-3">{ms.description}</p>
                    )}
                    {/* Progress bar */}
                    <div className="mb-2">
                      <div className="w-full bg-[#21262d] rounded-full h-2.5">
                        <div
                          className="bg-github-success h-2.5 rounded-full transition-all"
